...
class GlobalHead(nn.Module):
    ...
    def embed_rays(self, rays, shapes):
        ...
        rays_embedding = generate_fourier_features(rays_embedding, dim=self.camera_dim, max_freq=max(shapes) // 2, use_log=True, cat_orig=True)
...
class DepthHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.confidence_norms, self.confidence_out_layers = nn.ModuleList([]), nn.ModuleList([])
        for i, depth in enumerate(depths):
            blk_lst = nn.ModuleList([])
            for _ in range(depth):
                blk_lst.append(NystromBlock(hidden_dim // int(2 ** i), num_heads=num_heads // int(2 ** i), expansion=expansion, dropout=dropout))
            ...
            self.out_layers.append(nn.Conv2d(hidden_dim // int(2 ** (i + 1)), 1, 3, padding=1))
            self.confidence_norms.append(nn.LayerNorm(hidden_dim // int(2 ** (i + 1)))) 
            self.confidence_out_layers.append(nn.Conv2d(hidden_dim // int(2 ** (i + 1)), 1, 3, padding=1))
    ...
    def decode_depth(self, latents_16, rays, shapes):
        ...
        for i, (up, layers, rays_embedding) in enumerate(zip(self.ups, self.process_layers, rays)):
            ...
            latents = up(rearrange(latents + rays_embedding, "b (h w) c -> b c h w", h=shapes[0] * int(2 ** i), w=shapes[1] * int(2 ** i)).contiguous())
            out = rearrange(latents, "b (h w) c -> b h w c", h=shapes[0] * int(2 ** (1 + i)), w=shapes[1] * int(2 ** (1 + i)))
            out_features.append(out)

        for i, (norm, out_layer, features) in enumerate(zip(self.norms[::-1], self.out_layers[::-1], out_features[::-1])):
            features = norm(features)
            out_d = out_layer(features.permute(0, 3, 1, 2))
            outs.append(out_d)
        out = sum(F.interpolate(x, size=outs[0].shape[-2:], mode="bilinear", align_corners=False, antialias=True) for x in outs)
        out = out / len(outs)
        out = F.layer_norm(out.float(), out.shape[1:])
        out = out.clamp(-10.0, 10.0).exp().to(dtype, non_blocking=True)

        for i, (norm, out_layer, features) in enumerate(zip(self.confidence_norms[::-1], self.confidence_out_layers[::-1], out_features[::-1])):
            features = norm(features)
            out_c = out_layer(features.permute(0, 3, 1, 2))
            confidences.append(out_c)
        confidence = sum(F.interpolate(x, size=confidences[0].shape[-2:], mode="bilinear", align_corners=False, antialias=True) for x in confidences)
    ...
    def init_latents(self, features, shapes):
        ...
        latents_16 = features_16 + self.to_latents(flat_interpolate(features_16, old=self.shapes, new=shapes, antialias=False))
        return latents_16
    ...
    def forward(
        ...
        latents_16 = self.aggregate_16(init_latents_16, context=torch.cat(features, dim=1), pos_embed_context=pos_embed + level_embed)
...
class Decoder(nn.Module):
    ...
    def run_camera(self, cls_tokens, features, pos_embed, original_shapes, rays_gt):
        ...
        cls_tokens_splits = torch.tensor(
            [x.shape[-1] for x in cls_tokens], device=features.device,
            requires_grad=False, dtype=features.dtype,
        )
        cls_tokens = torch.cat(cls_tokens, dim=-1)
        cls_tokens = self.camera_token_adapter(cls_tokens, cls_tokens_splits)
        cls_tokens = torch.cat(torch.chunk(cls_tokens, len(cls_tokens_splits), dim=-1), dim=1)
    ...
    def forward(self, inputs, image_metas) -> torch.Tensor:
        ...
        input_shapes = [level_shapes[i] for i, (start, end) in enumerate(self.slices_encoder) for _ in range(end - start)]
        ...
        features_splits = torch.tensor([x.shape[-1] for x in features_flat], device=device, requires_grad=False, dtype=torch.float32)
        ...
        level_embed = torch.cat([
            self.level_embed_layer(self.level_embeds)[i : i + 1].unsqueeze(0).repeat(B, common_shape[0] * common_shape[1], 1)
